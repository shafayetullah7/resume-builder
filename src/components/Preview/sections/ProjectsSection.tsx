...
const ProjectsSection: React.FC<ProjectsSectionProps> = ({ projects, label, tokens, visible }) => {
  ...
  const pillClass =
    tokens.skillsStyle === 'pills'
      ? `${tokens.pillSize} px-2 py-0.5 rounded border font-medium`
      : '';
        ...
        {projects.map((proj) => (
          <div key={proj.id} className="break-inside-avoid">
            <div className="flex justify-between items-baseline">
              <span className={`${tokens.bodySize} font-semibold text-gray-800`}>{proj.name}</span>
              <span className={`${tokens.metaSize} text-gray-500`}>
                {formatDateRange(proj.startDate || '', proj.endDate || '', false)}
              </span>
            </div>
            {proj.description && (
              <p className={`${tokens.bodySize} text-gray-700 mt-0.5 leading-snug`}>{proj.description}</p>
            )}
            {proj.technologies && proj.technologies.length > 0 && (
              tokens.skillsStyle === 'comma' ? (
                <p className={`${tokens.metaSize} text-gray-600 mt-1 leading-snug`}>{proj.technologies.join(', ')}</p>
              ) : (
            ...
            {proj.links && proj.links.length > 0 && (
              <div className={`flex flex-col gap-1 ${tokens.metaSize} mt-1`}>
