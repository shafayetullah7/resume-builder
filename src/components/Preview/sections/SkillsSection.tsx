      ...
      <div className="flex flex-col gap-2">
        {skills.map((skillCat) => (
          <div key={skillCat.id} className="break-inside-avoid">
            <span className={`${tokens.bodySize} font-semibold text-gray-800 block mb-0.5`}>{skillCat.category}</span>
            {tokens.skillsStyle === 'comma' ? (
              <p className={`${tokens.metaSize} text-gray-700 leading-snug`}>{skillCat.skills.join(', ')}</p>
            ) : (
