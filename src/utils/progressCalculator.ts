...
const calculateProgress = (data: ResumeData): { overall: number; sections: SectionProgress[] } => {
    ...
    const hasExperienceContent = data.experience.some(
        (e) => e.description || (e.bullets && e.bullets.some((b) => b.trim()))
    );
    const experienceFilled =
        ...
        (hasExperienceContent ? 1 : 0) +
        (data.experience.some((e) => e.location) ? 1 : 0);
    sections.push({
        name: 'Experience',
        filled: experienceFilled,
        total: 5,
        percentage: Math.round((experienceFilled / 5) * 100),
    });
    ...
    const certificationsFilled =
        ...
        (data.certifications.some((c) => c.issuer) ? 1 : 0);
    sections.push({
        name: 'Certifications',
        filled: certificationsFilled,
        total: 3,
        percentage: Math.round((certificationsFilled / 3) * 100),
    });
